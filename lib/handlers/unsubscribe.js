...
function handleUnsubscribe (client, packet, done) {
  ...
  if (isSpatial(topic)) {
    ...
    console.log('Spatial unsubscribe <'+data.x+','+data.y+','+data.radius+','+data.channel+'>')

    client.broker.matcher._unsubscribe(client.id, data.x, data.y, data.radius, data.channel);
