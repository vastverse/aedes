...
function Client (broker, conn, req) {
  ...
  this.deliver0 = function deliverQoS0 (_packet, cb) {
    ...
    //console.log('Client.js::deliver0 => toForward <'+toForward+'>')
    if (toForward) {
      // Give nodejs some time to clear stacks, or we will see
      // "Maximum call stack size exceeded" in a very high load
     // console.log('Client.js::deliver0 => toForward branch')

      setImmediate(() => {
        const packet = new Packet(toForward, broker)
        packet.qos = 0
        //console.log('Client.js::setImmediate => writing Packet to client <'+that.id+'>')
      ...
      //console.log("Client.js::deliver0 => SetImmediate callback")
      setImmediate(cb)
    }
  }
  ...
  this.deliverQoS = function deliverQoS (_packet, cb) {
    ...
    //console.log('Client::deliverQoS => this.deliverQoS called')
    if (_packet.qos === 0) {
     // console.log('Client::deliverQoS => this.deliver0 will be called for client <'+this.id+'>')
